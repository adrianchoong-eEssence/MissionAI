...
class SubmissionStorageTests(unittest.TestCase):
    ...
    def test_photo_url_preserves_existing_and_resolves_storage_reference(self):
        self.assertEqual(
            get_photo_url("https://example.com/photo.jpg"),
            "https://example.com/photo.jpg",
        )
        with patch(
            "data.google_drive._signed_photo_url",
            return_value="https://signed.example/photo.jpg",
        ):
            result = get_photo_url(
                "supabase://exos-submissions/EVT/M01/photo.jpg"
            )
        self.assertEqual(result, "https://signed.example/photo.jpg")

...
def _persist_session(player: dict, event_id: str, device_id: str) -> None:
    ...
    for key in list(st.query_params):
        if str(key).casefold() in {
            ...
            "enrollmentcredential", "participant_name",
        }:
...
def _render_reconnect(runtime, event_id: str, join_code: str, credential: str, device_id: str) -> None:
    if not credential:
        st.info("Securing this device for reconnect…")
        return
    if st.button("RECONNECT / CONTINUE", width="stretch", key="aia_random_reconnect"):
        ...
        _persist_session(player, event_id, device_id)
        st.rerun()
...
def render_aia_random_registration() -> None:
    ...
    credential = participant_enrollment_credential(event_id, key=f"aia_random_credential_{event_id}")

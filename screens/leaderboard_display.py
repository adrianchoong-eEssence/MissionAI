...
from screens.maxis_uat_projector import MAXIS_UAT_EVENT_ID, render_maxis_uat_projector
...
def show_leaderboard_display(db=None, event_id="", standalone=False):
    ...
    if db.is_theme_park_race_event(event):
        if str(event_id) == MAXIS_UAT_EVENT_ID:
            render_maxis_uat_projector(db, event_id)
            return

...
def show_participant():
    ...
    if "participant_event_id" not in st.session_state:
        ...
        if st.button(
            ...
            player = db.join_player(
                event["EventID"],
                participant_name,
            )

            ai = db.assign_ai_facilitator(
                player["Team"]
            )
    ...
    with col2:
        st.metric(
            "Points",
            st.session_state["participant_points"],
        )
    ...
    if mission is None:
        ...
        st.info(
            "Waiting for facilitator to launch a mission..."
        )
        ...
        if existing_submission:
            ...
            st.markdown(
                f"[View Submission]({existing_submission['ImageURL']})"
            )
    ...
    st.subheader(
        f"🤖 {st.session_state['ai_name']}"
    )

    if st.session_state["ai_greeting"]:

        st.info(
            st.session_state["ai_greeting"]
        )
    ...
    prompt = st.chat_input(
        f"Ask {st.session_state['ai_name']}..."
    )

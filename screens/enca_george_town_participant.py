...
def render_enca_george_town_participant(*, event_id: str, join_code: str) -> None:
    ...
    if runtime is not None and _has_resume_token():
    ...
    with st.form("enca_general_registration", clear_on_submit=False):
        first_name = st.text_input("First / Given Name", autocomplete="given-name", disabled=runtime is None)
        last_name = st.text_input("Last / Family Name", autocomplete="family-name", disabled=runtime is None)
        entered = st.form_submit_button("ENTER MISSION AI", type="primary", width="stretch", disabled=runtime is None)
    if entered and runtime is not None:
        name = normalise_join_name(first_name, last_name)
        if not first_name.strip() or not last_name.strip():
            st.error("Enter both your first / given and last / family name.")
        elif _event_is_available(runtime, event_id, join_code):
            try:
    ...
    with st.form("enca_hod_personal_key", clear_on_submit=False):
        personal_key = st.text_input("Personal Key", type="password", autocomplete="one-time-code", disabled=runtime is None)
        claimed = st.form_submit_button("CONTINUE", width="stretch", disabled=runtime is None)
    if claimed and runtime is not None:
